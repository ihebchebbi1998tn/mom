...
switch ($method) {
    case 'GET':
        ...
        try {
            $sql = "SELECT r.*, u.name as user_name, u.email as user_email, u.phone as user_phone, 
                           p.title as pack_title, p.price as pack_price 
                    FROM mom_requests r 
    ...
    case 'PUT':
        ...
        if (!$id || !$status) {
            echo json_encode(['success' => false, 'message' => 'Request ID and status are required'], JSON_UNESCAPED_UNICODE);
            exit;
        }
        ...
        try {
            $stmt = $db->prepare("UPDATE mom_requests SET status = ?, admin_notes = ?, admin_response_date = CURRENT_TIMESTAMP WHERE id = ?");
