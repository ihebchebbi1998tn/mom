...
try {
    ...
    // Check 1: Direct workshop access
    $stmt = $db->prepare("
        SELECT * FROM mom_workshop_requests 
        WHERE user_id = :user_id AND workshop_id = :workshop_id AND status = 'accepted'
    ");
    $stmt->execute([':user_id' => $user_id, ':workshop_id' => $workshop_id]);
    ...
    if ($directAccess) {
        echo json_encode([
            ...
            'hasAccess' => true,
            'accessType' => 'direct',
            'message' => 'Direct workshop access'
        ]);
        exit;
    }
    ...
    $stmt = $db->prepare("
        ...
        FROM mom_packs p
        INNER JOIN mom_pack_workshop_links pwl ON p.id = pwl.pack_id
        INNER JOIN mom_requests r ON p.id = r.pack_id
        WHERE pwl.workshop_id = :workshop_id 
        AND r.user_id = :user_id 
        AND r.status = 'accepted'
        LIMIT 1
    ");
    $stmt->execute([':workshop_id' => $workshop_id, ':user_id' => $user_id]);
    ...
    echo json_encode([
        'success' => true,
        'hasAccess' => false,
        'message' => 'No access to this workshop'
    ]);
