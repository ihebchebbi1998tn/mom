...
const ConsultationAvailabilityManagement = () => {
  ...
  const saveDefaultSettings = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setIsEditingDefault(false);
        toast({ title: "Succès", description: "Paramètres par défaut sauvegardés avec succès" });
      } else {
        toast({ title: "Erreur", description: data.message, variant: "destructive" });
      }
    } catch (error) {
      toast({ title: "Avertissement", description: "Les paramètres seront sauvegardés en production", variant: "destructive" });
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const body = editingId 
        ? { id: editingId, status, max_reservations: maxReservations, notes }
        : { date: formattedDate, status, max_reservations: maxReservations, notes };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (data.success) {
        toast({ title: "Succès", description: "Enregistrement supprimé avec succès" });
        setTimeout(async () => {
          await fetchAvailabilities();
        }, 100);
      } else {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      full: { label: "Complet", variant: "secondary" as const },
    ...
    <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {!isEditingDefault ? (
                  ...
                  <div className="space-y-3">
                    <Input
                      type="number"
                      value={tempDefaultMaxReservations}
                      onChange={(e) => setTempDefaultMaxReservations(parseInt(e.target.value) || 3)}
                      min="1"
                      max="20"
                      placeholder="Nombre par défaut de réservations"
                    />
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Gestion des Disponibilités de Rendez-vous (Personnaliser des dates spécifiques)</CardTitle>
          <Button 
            variant="outline" 
            size="sm" 
          ...
          <div className="space-y-6">
            ...
            <div className={`border-2 rounded-lg p-6 transition-all duration-300 ${
              selectedDate 
                ...
                : 'border-muted/50 bg-muted/20 opacity-50 pointer-events-none'
            }`}>
              <div className="mb-4">
                ...
                <p className="text-muted-foreground text-sm">
                  {selectedDate 
                    ? 'Configurez maintenant les paramètres de disponibilité pour cette date'
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setMaxReservations(parseInt(e.target.value) || defaultMaxReservations)}
                      min="1"
                      max="20"
                      placeholder={`Par défaut: ${defaultMaxReservations}`}
                    />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    placeholder="Ajoutez des notes spéciales pour cette date (optionnel)"
                    rows={3}
                  />
                </div>
