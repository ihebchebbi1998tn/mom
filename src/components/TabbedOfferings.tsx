...
const TabbedOfferings = () => {
  ...
  // Check URL parameters on mount to switch to appropriate tab
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    const tab = params.get('tab');
    if (tab && ['packs', 'courses', 'workshops'].includes(tab)) {
      setActiveTab(tab);
  ...
  const [videoCurrentTimes, setVideoCurrentTimes] = useState<{
    [key: number]: number;
  }>({});
  ...
  const fetchData = async () => {
    try {
      const [packsResponse, subPacksResponse, workshopsResponse] = await Promise.all([fetch('https://spadadibattaglia.com/mom/api/course_packs.php'), fetch('https://spadadibattaglia.com/mom/api/sub_packs.php'), fetch('https://spadadibattaglia.com/mom/api/workshops.php')]);
  ...
  return <section id="offerings" className="py-20 section-gradient" ref={sectionRef}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full" dir="rtl">
          <div className="flex justify-center mb-12">
            <TabsList className="bg-gray-100 p-1 rounded-full inline-flex gap-1">
              <TabsTrigger value="packs" className="rounded-full px-8 py-3 data-[state=active]:bg-gradient-to-r data-[state=active]:from-pink-500 data-[state=active]:to-rose-500 data-[state=active]:text-white transition-all duration-300">
                الباقات
              </TabsTrigger>
              <TabsTrigger value="courses" className="rounded-full px-8 py-3 data-[state=active]:bg-gradient-to-r data-[state=active]:from-pink-500 data-[state=active]:to-rose-500 data-[state=active]:text-white transition-all duration-300">
                الدورات
              </TabsTrigger>
              <TabsTrigger value="workshops" className="rounded-full px-8 py-3 data-[state=active]:bg-gradient-to-r data-[state=active]:from-pink-500 data-[state=active]:to-rose-500 data-[state=active]:text-white transition-all duration-300">
                الورشات
              </TabsTrigger>
            </TabsList>
          </div>
          ...
          <TabsContent value="packs">
            {loading ? <div className="text-center py-12">
              ...
              </div> : coursePacks.length === 0 ? <div className="text-center py-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto lg:[&>*:last-child:nth-child(3n+1)]:col-start-2">
                  {visiblePacks.map((pack, index) => <div key={pack.id} ref={el => packRefs.current[pack.id] = el} className={`group relative transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{
                transitionDelay: isVisible ? `${300 + index * 200}ms` : '0ms'
              }} onMouseEnter={() => !isMobile && setHoveredPack(pack.id)} onMouseLeave={() => !isMobile && setHoveredPack(null)}>
                      <div className={`bg-card rounded-xl shadow-lg hover:shadow-xl transition-all duration-500 overflow-hidden flex flex-col h-full ${pack.title.includes('الذهني') ? 'border-2 border-yellow-400 relative before:absolute before:inset-0 before:rounded-xl before:p-[2px] before:bg-gradient-to-r before:from-yellow-300 before:via-amber-400 before:to-yellow-300 before:-z-10 before:animate-shimmer after:absolute after:inset-0 after:rounded-xl after:shadow-[0_0_20px_rgba(251,191,36,0.5)] after:-z-10 after:animate-pulse' : 'border border-border/50'}`}>
                        {/* Image/Video Section */}
                        <div className="relative h-48 overflow-hidden perspective-1000">
                          <div className={`relative w-full h-full transition-all duration-700 transform-style-3d ${playingVideo === pack.id && pack.intro_video_url ? 'rotate-y-180' : ''}`}>
                            {/* Front - Image */}
                            <div className="absolute inset-0 backface-hidden">
                              <img src={pack.image_url || "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=400&h=200&fit=crop&crop=center"} alt={pack.title} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                              {/* Hover Indicator - Desktop Only */}
                              {!isMobile && pack.intro_video_url && <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/40 to-transparent p-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            ...
                            {/* Back - Video */}
                            {pack.intro_video_url && <div className="absolute inset-0 backface-hidden rotate-y-180">
                                <video ref={el => {
                          ...
                          if (el) {
                            ...
                            el.addEventListener('timeupdate', () => {
                              setVideoCurrentTimes(prev => ({
                                ...prev,
                                [pack.id]: el.currentTime
                              }));
                            });
                          }
                        }} src={pack.intro_video_url} className="w-full h-full object-contain cursor-pointer" muted={true} loop playsInline preload="metadata" style={{
                          contentVisibility: 'auto'
                        }} onClick={() => resetControlsTimeout(pack.id)} />
                                
                                {/* Video Controls Overlay */}
                                {playingVideo === pack.id && <div onMouseMove={() => resetControlsTimeout(pack.id)} onTouchStart={() => resetControlsTimeout(pack.id)} className="absolute inset-0 z-20">
                                    {/* Floating text - Arabic instruction - smaller with pink background */}
                                    {mutedVideos[pack.id] !== false && showControls[pack.id] !== false && <div className="absolute -top-12 left-1/2 -translate-x-1/2 -ml-4 z-30 animate-bounce">
                          ...
                          {isMobile && loadingVideos[pack.id] && <div className="absolute inset-0 flex items-center justify-center bg-black/50 z-20">
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                          ...
                          {!isMobile && pack.intro_video_url && <div className="mb-3 flex items-center gap-2 text-primary text-sm bg-primary/10 rounded-lg px-3 py-2 border border-primary/20 cursor-pointer hover:bg-primary/20 hover:scale-105 transition-all duration-300" onClick={e => {
                      e.stopPropagation();
                      setPlayingVideo(pack.id);
                          ...
                          <div className="space-y-2 mb-4 flex-1">
                            {pack.modules && pack.modules.split(',').slice(0, 3).map((module, idx) => <div key={idx} className="flex items-start gap-2 text-sm text-muted-foreground">
                                <CheckCircle className="w-4 h-4 text-primary flex-shrink-0 mt-0.5" />
                                <span className="line-clamp-1">{module.trim()}</span>
                              </div>)}
                            {pack.modules && pack.modules.split(',').length > 3 && <div className="text-sm text-primary font-medium">
                                <span dir="ltr">+{pack.modules.split(',').length - 3}</span> مواضيع أخرى
                              </div>}
                          </div>

                          {/* Button */}
                          <Button size="sm" className={`w-full hover:scale-105 transition-all duration-300 relative overflow-hidden ${pack.title.includes('الذهني') ? 'bg-gradient-to-r from-yellow-400 via-amber-500 to-yellow-400 text-white font-bold hover:shadow-[0_0_30px_rgba(251,191,36,0.7)] border-2 border-yellow-300 before:absolute before:top-0 before:left-[-100%] before:w-full before:h-full before:bg-gradient-to-r before:from-transparent before:via-white/40 before:to-transparent before:animate-shimmer before:skew-x-12' : 'btn-hero hover:shadow-[0_0_30px_rgba(233,30,99,0.5)] before:absolute before:top-0 before:left-[-100%] before:w-full before:h-full before:bg-gradient-to-r before:from-transparent before:via-white/30 before:to-transparent before:animate-shimmer before:skew-x-12'}`} onClick={() => navigate(`/pack/${pack.id}`)}>
                            للمزيد من التفاصيل
                ...
                {hasMorePacks && <div className="flex justify-center mt-8">
          ...
          <TabsContent value="courses">
            {loading ? <div className="text-center py-12">
              ...
              </div> : subPacks.length === 0 ? <div className="text-center py-12">
                <BookOpen className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
                <p className="text-muted-foreground">لا توجد دورات متاحة حالياً</p>
              </div> : <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-7xl mx-auto">
                  {visibleSubPacks.map((subPack, index) => <div key={subPack.id} className={`group relative transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{
                transitionDelay: isVisible ? `${300 + index * 200}ms` : '0ms'
              }}>
                      <div className="bg-card rounded-xl shadow-lg hover:shadow-xl transition-all duration-500 overflow-hidden flex flex-col h-full border border-border/50">
                        ...
                        <div className="relative h-48 overflow-hidden">
                          <img src={subPack.banner_image_url || "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=400&h=200&fit=crop&crop=center"} alt={subPack.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                        </div>
                ...
                {hasMoreSubPacks && <div className="flex justify-center mt-8">
                ...
                <div className={`text-center mt-16 transition-all duration-1000 delay-700 ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-8 scale-95'}`}>
                  <div className="card-elegant p-8 max-w-2xl mx-auto bg-gradient-to-r from-primary/5 to-primary-light/10">
                    <h3 className="text-2xl font-bold text-foreground mb-4">
                      لديك فكرة لدورة جديدة؟
                    </h3>
                    <p className="text-muted-foreground mb-6">
                      نحن نستمع لاحتياجاتكم ونطور دورات جديدة باستمرار. شاركينا أفكارك واقتراحاتك
                    </p>
                    <Button variant="outline" className="btn-outline" onClick={() => {
                  const message = encodeURIComponent("لدي اقتراح لدورة جديدة أود مناقشتها معكم");
          ...
          <TabsContent value="workshops">
            {loading ? <div className="text-center py-12">
              ...
              </div> : workshops.length === 0 ? <div className="text-center py-12">
                <BookOpen className="w-16 h-16 text-muted-foreground mx-auto mb-4" />
                ...
                <div className="grid xl:grid-cols-3 lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
                  {workshops.map((workshop, index) => <div key={workshop.id} className={`card-elegant overflow-hidden flex flex-col h-full transition-all duration-1000 hover:scale-105 hover:shadow-xl ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{
                transitionDelay: isVisible ? `${300 + index * 150}ms` : '0ms'
              }}>
                      {/* Image Section */}
                      <div className="relative h-48 overflow-hidden">
                        <img src={workshop.image_url || "https://images.unsplash.com/photo-1481627834876-b7833e8f5570?w=400&h=200&fit=crop&crop=center"} alt={workshop.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                      </div>

                      {/* Content Section */}
                      <div className="p-6 flex-1 flex flex-col">
                        <h3 className={`text-xl font-bold text-foreground mb-3 ${getTextAlignmentClasses(workshop.title)} ${getContainerDirection(workshop.title)}`} dir={getTextDirection(workshop.title)} style={{
                    unicodeBidi: 'plaintext'
                  }}>
                          {workshop.title}
                        </h3>

                        <div className={`text-muted-foreground mb-4 leading-relaxed flex-grow ${getTextAlignmentClasses(workshop.description)} ${getContainerDirection(workshop.description)}`} dir={getTextDirection(workshop.description)} style={{
                    unicodeBidi: 'plaintext'
                  }}>
                          {workshop.description.includes('-') ? (
                            <ul className="space-y-2">
                              {workshop.description.split('-').filter(item => item.trim()).map((item, idx) => (
