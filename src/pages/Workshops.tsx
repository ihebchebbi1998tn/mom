...
const Workshops = () => {
  ...
  const [workshopPackAccess, setWorkshopPackAccess] = useState<{
    [key: number]: {
      hasAccess: boolean;
      packName?: string;
    };
  ...
  return <div className="min-h-screen w-full bg-gradient-to-br from-pink-50 via-white to-purple-50">
      ...
      <div className="flex h-[calc(100vh-80px)] w-full">
        ...
        <main className="flex-1 px-4 md:px-8 py-6 lg:py-8 transition-all duration-200 overflow-y-auto overflow-x-hidden">
            ...
            {/* Workshops Grid */}
            {workshops.length > 0 ? <div className="grid xl:grid-cols-3 lg:grid-cols-2 gap-8 max-w-7xl mx-auto mb-8">
                {workshops.map((workshop, index) => <Card key={workshop.id} className={`bg-white/80 backdrop-blur-sm border-0 shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{
            transitionDelay: `${index * 100}ms`
          }}>
                    {/* Workshop Image */}
                    {workshop.image_url && <div className="w-full h-48 overflow-hidden">
                    ...
                    <CardContent className="p-6">
                      {/* Workshop Header */}
                      <div className="mb-4">
                        <h3 className={`text-xl font-bold text-primary mb-2 ${getTextAlignmentClasses(workshop.title)} ${getContainerDirection(workshop.title)}`} dir={getTextDirection(workshop.title)} style={{
                  unicodeBidi: 'plaintext'
                      ...
                      {/* Description with line breaks at "-" */}
                      <div className={`text-muted-foreground mb-4 leading-relaxed ${getTextAlignmentClasses(workshop.description)} ${getContainerDirection(workshop.description)}`} dir={getTextDirection(workshop.description)} style={{
                unicodeBidi: 'plaintext',
                whiteSpace: 'pre-line'
              }}>
                        {workshop.description.split('-').map((part, idx, arr) => <span key={idx}>
                            {idx > 0 ? '* ' : ''}{part.trim()}
                            {idx < arr.length - 1 && <br />}
                          </span>)}
                      </div>
              ...
              {(() => {
                ...
                if (user?.role === 'admin') {
                  return <Button className="bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 w-full" onClick={() => handleAccessWorkshop(workshop.id)}>
                            الدخول إلى الورشة
                ...
                if (requestStatus === 'accepted') {
                  return <div className="space-y-2">
                              ...
                              <Button className="bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 w-full" onClick={() => handleAccessWorkshop(workshop.id)}>
                                الدخول إلى الورشة
                                <ArrowLeft className="w-4 h-4 mr-2" />
                              </Button>
                            </div>;
                ...
                if (packAccess && packAccess.hasAccess) {
                  return <div className="space-y-2">
                              ...
                              <Button className="bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 w-full" onClick={() => handleAccessWorkshop(workshop.id)}>
                                الدخول إلى الورشة
                                <ArrowLeft className="w-4 h-4 mr-2" />
                              </Button>
                            </div>;
                ...
                if (requestStatus === 'pending') {
                  return <div className="space-y-2">
                              ...
                              {!hasUploadedReceipt && showReceiptUpload[workshop.id] && <ReceiptUpload requestId={showReceiptUpload[workshop.id]} onUploadComplete={handleReceiptUploadComplete} apiEndpoint="workshop_requests" />}
                ...
                if (requestStatus === 'rejected') {
                  return <Badge className="w-full bg-red-100 text-red-700 hover:bg-red-100 justify-center py-2">
                              <AlertCircle className="w-4 h-4 ml-2" />
                ...
                // No request yet
                return <Button className="bg-gradient-to-r from-pink-500 to-rose-500 hover:from-pink-600 hover:to-rose-600 w-full" onClick={() => handleWorkshopRequest(workshop.id)} disabled={actionLoading === String(workshop.id)}>
